<template>
  <div class="floating-panel">
    <div class="panel-header"><h3>前沿技术</h3><button class="panel-close-btn" @click="$router.push('/overview')">✕</button></div>
    <div class="panel-body"><GaussianSplatContent :model-value="gaussianLayer" @update:model-value="val => $emit('update:gaussianLayer', val)" /></div>
  </div>
</template>

<script setup>
import GaussianSplatContent from '../components/GaussianSplatContent.vue'
defineProps({ gaussianLayer: Boolean })
defineEmits(['update:gaussianLayer'])
</script>
